...
void putfloat(float a) {
  ...
  oss << std::hexfloat << std::setprecision(std::numeric_limits<float>::max_digits10) << a;
  ...
  if (ppos != std::string::npos) {
      size_t dot = s.find('.');
      if (dot != std::string::npos && dot < ppos) {
          size_t i = ppos;
          while (i > dot + 1 && s[i - 1] == '0') {
              --i;
          }
          if (i == dot + 1) {
              s.erase(dot, ppos - dot);
          }
          else {
              s.erase(i, ppos - i);
          }
  ...
  fprintf(ir::reopen_output_file, "%s", s.c_str());
}
...
void putfarray(int n, float a[]) {
    ...
    for (int i = 0; i < n; i++) {
        std::ostringstream oss;
        oss << std::hexfloat << std::setprecision(std::numeric_limits<float>::max_digits10) << a[i];
        std::string s = oss.str();
        size_t ppos = s.find('p');
        if (ppos == std::string::npos) {
            ppos = s.find('P');
